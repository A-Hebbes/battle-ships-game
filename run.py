...
def display_rules():
    ...
    print("- A successful hit activates sonar scan of adjacent squares")
    print("- Game ends if all opponent's ships are sunk or no more turns.")
    print("\nLet's get started!\n")
...
def get_target_location(board_size):
    ...
    while not row.isdigit() or not 1 <= int(row) <= board_size:
        ...
        row = input(f'Enter a row number to target (1-{board_size}): ')
    column = input(
        f'Enter column letter (A-{chr(ord("A") + board_size - 1)}): '
        ).upper()
    while (
        len(column) != 1 or
        column < 'A' or
        column > chr(ord("A") + board_size - 1)
    ):
        print("---------------------------")
        print("Enter a valid column letter")
        print("---------------------------")
        column = (
            input(f'Enter target col. (A-{chr(ord("A") + board_size - 1)}): ')
            .upper()
        )
...
while turns > 0:
    ...
    if BLUE_BOARD[row][column] == '-':
        ...
        if check_game_over(RED_BOARD, BLUE_BOARD):
            ...
            print('If you would like to play again click')
            print('the "Run Program" button at the top of the page.')
            break
    else:
        print("^^^^^^^^^^^^^^^")
        print("You missed! :(")
    ...
    if turns <= 0:
        ...
        print('Better Luck Next Time. Gooodbye.')
        print('If you would like to play again click')
        print('the "Run Program" button at the top of the page.')
        break
